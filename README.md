...
**佐證圖表：** <span style="color:red">這是紅色文字</span>
- `chart_fps_by_configuration.png` — 三種 backend 的 FPS 長條圖（含課程基準參考線）
- `chart_latency_distribution.png` — p50/p95/p99 分佈grouped bar chart
...
**分析：**
- `--min-area 500` 是跳過率（80%）與漏偵率（1%）的良好平衡點
- `--min-area` 越大，跳過率越高但漏偵率也越高，為典型的 tradeoff
- 靜態場景的 missed detection 主要來自 MOG2 的設計限制：
  只偵測**像素變化**，靜止物件（如固定放置的杯子）不會觸發，這是 motion gating 的根本限制

**佐證圖表：** <span style="color:red">這是紅色文字</span>
- `chart_motion_gate_tradeoff.png` — 三種 min-area 下 skip rate vs missed % vs false % 的折線圖（建議新增）

---

### 4. 準確度量測（COCO val2017, 50 張隨機子集）<span style="color:red">這是紅色文字</span>
...
**Confidence Threshold Sweep：** <span style="color:red">這是紅色文字</span>
...
**分析：** <span style="color:red">這是紅色文字</span>
...
### [組員姓名]<span style="color:red">這是紅色文字</span>
